...
export async function fetchListings(fetch: Request, url: string): Promise<Listing[]> {
  const response = await fetch(url);
  return response.json();
}

export async function fetchRedditApi(fetch: Request) {
  const listings = await fetchListings(fetch, URL);
